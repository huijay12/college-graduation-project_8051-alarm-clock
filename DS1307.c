...
#define WEEKDAY_ADDRESS  0x03  // Address to access Ds1307 DAY register
 ...
 * description  :This function is used to set Date(dd,mm,yy) into the Ds1307 RTC.
                 ...
                 Stop the I2c communication.
----------------------------------------------------------------------------------*/
void DS1307_SetDate(unsigned char wdwd, unsigned char dd, unsigned char mm, unsigned char yy)
//void DS1307_SetDate(unsigned char dd, unsigned char mm, unsigned char yy)
{
	//wdwd = (((unsigned char)(wdwd/10))<<4)|((unsigned char)(wdwd%10));

 	I2C_Start();            // Start I2C communication

	DS1307_Write(DS1307_ID);	// connect to DS1307 by sending its ID on I2c Bus
	DS1307_Write(WEEKDAY_ADDRESS);		// Request DAY RAM address at 04H
	//DS1307_Write(DATE_ADDRESS);		// Request DAY RAM address at 04H

	DS1307_Write(wdwd);			// Write date on RAM address 03H
 ...
 * description  :This function is used to get the Date(y,m,d) from Ds1307 RTC.
                 ...
                 Stop the I2c communication.
-----------------------------------------------------------------------------------*/
void DS1307_GetDate(unsigned char *wd_ptr,unsigned char *d_ptr,unsigned char *m_ptr,unsigned char *y_ptr)
{
    I2C_Start();            // Start I2C communication

	DS1307_Write(DS1307_ID);	// connect to DS1307 by sending its ID on I2c Bus
	DS1307_Write(WEEKDAY_ADDRESS);		// Request WEEKDAY RAM address at 04H
  ...
  *wd_ptr = DS1307_Read(); I2C_Ack();	 // read Day and return Positive ACK
